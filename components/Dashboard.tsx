...
import { Invoice, InvoiceStatus, DocumentType, ExpenseRecord, BusinessProfile } from '../types';
...
import { useInvoiceStore, useExpenseStore, useBusinessStore} from '../services/store';

const Dashboard: React.FC = () => {

   const invoices = useInvoiceStore((state:any) => state.invoices);
   const expenses = useExpenseStore((state:any) => state.expenses);
  ...
  const statCards = [
    { label: 'Settled Revenue', value:150, color: 'text-emerald-600', bg: 'bg-emerald-50', icon: IndianRupee  },
    { label: 'Total Expenses', value: 300, color: 'text-rose-500', bg: 'bg-rose-50', icon: TrendingDown }
  ];
